...
def run_optimization_with_rl(trainer, env, exp_dir: str):
    ...
    # بهترین پارامترها از آموزش
    optimal_params = {
        'pop_size': 50,  # مقدار معقول
        'max_iter': 50,  # برای اجرای سریع‌تر
        'low': -3.0,
        'high': 3.0,
        'eq_pool_size': 4,
        'p_mut': 0.1,
    ...
    print(f"\nUsing optimized parameters: {optimal_params}")
...
def save_results(exp_dir: str, results: dict):
    ...
    # تبدیل همه مقادیر به فرمت قابل ذخیره در JSON
    def convert_to_serializable(obj):
        """تبدیل انواع مختلف به فرمت قابل ذخیره در JSON"""
        import numpy as np
        
        if isinstance(obj, (np.float32, np.float64, np.int32, np.int64)):
            return float(obj)  # تبدیل به float استاندارد
        elif isinstance(obj, np.ndarray):
            return obj.tolist()  # تبدیل آرایه numpy به لیست
        elif isinstance(obj, dict):
            return {key: convert_to_serializable(value) for key, value in obj.items()}
        elif isinstance(obj, list):
            return [convert_to_serializable(item) for item in obj]
        elif hasattr(obj, '__dict__'):  # برای objectها
            return obj.__dict__
        else:
            return obj
    ...
    # ذخیره آمار
    stats = {
        'seed_sp': convert_to_serializable(results['seed_sp']),
        'optimized_sp': convert_to_serializable(results['optimized_sp']),
        'refined_sp': convert_to_serializable(results['refined_sp']),
        'improvement': convert_to_serializable(results['improvement']),
        'optimal_params': convert_to_serializable(results['optimal_params']),
...
def compare_methods(fasta_path: str, exp_dir: str):
    ...
    # 4. EO-Pro با RL (بارگذاری از نتایج ذخیره شده)
    print("\n4. EO-Pro with RL-optimized parameters...")
    ...
    if os.path.exists(results_path):
        try:
            ...
            # استفاده از refined_sp که پس از local refinement است
            rl_sp = rl_results.get('refined_sp', rl_results.get('optimized_sp', 0))
            results['EO-Pro (RL)'] = {
                'sp': rl_sp,
                'alignment': []  # اگر فایل alignment موجود است می‌توانید بارگذاری کنید
            }
            print(f"   SP: {rl_sp:.2f}")
            
            improvement = rl_results.get('improvement', 0)
            if improvement > 0:
                print(f"   ✓ Improvement over seed: +{improvement:.2f}")
            else:
                print(f"   ✗ Worse than seed: {improvement:.2f}")
        except Exception as e:
    ...
    for method, data in sorted_results:
        print(f"{method:<20}: {data['sp']:>12.2f}")
