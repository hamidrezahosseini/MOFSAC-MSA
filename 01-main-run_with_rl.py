...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("RNA SEQUENCE ALIGNMENT PIPELINE - IMPROVED VERSION")
    print("="*70)
    
    # بررسی وجود فایل ورودی
    if not os.path.exists("input.fasta"):
        print("\nCreating sample input.fasta...")
        sample_seqs = [
    ...
    # اجرای پایپ‌لاین
    try:
        print("\n🚀 Starting improved pipeline...")
        final_alignment, results, all_metrics, exp_dir = run_final_pipeline_improved()
        ...
        if all_metrics:
            ...
            print(f"\n🏆 FINAL RESULTS:")
            print(f"   Best Method: {best_method} (SP={best_sp:.0f})")
            ...
            for method_name, metrics in all_metrics.items():
                ...
                improvement = ""
                if 'MAFFT' in all_metrics and method_name != 'MAFFT':
                    base_sp = all_metrics['MAFFT']['sp_score']
                    imp = sp_val - base_sp
                    if imp != 0:
                        improvement = f" ({imp:+.0f} from MAFFT)"
                
                print(f"   {method_name:<20}: SP={sp_val:>6.0f}, TC={tc_val:>6.3f}, "
                      f"Gap%={gap_pct:>5.1f}%, Time={time_val:>6.2f}s{improvement}")
        
        print(f"\n🎉 Pipeline completed successfully!")
        print(f"📁 Check {exp_dir} for all results and visualizations.")
        ...
        print("\n🔧 TROUBLESHOOTING:")
        print("   1. Check if MAFFT and ClustalW are installed correctly")
        print("   2. Verify the input.fasta file format")
