...
Creates challenging search space -> SAC clearly outperforms fixed EO.
...
class FlexibleGapRepresentation:
    """
    Each initial gap gives a dimension. Vector value v in [-1,1]:
      - if |v| < 0.2 : no change
      - elif v >= 0.2: shift right by (v * max_shift), but if v > 0.8: delete gap
      - elif v <= -0.2: shift left by (|v| * max_shift), but if v < -0.8: insert new gap at a random non-gap column in that row
    All decisions deterministic given vector.
    """
    def __init__(self, seed_alignment, max_shift=15):
        ...
        # store original characters for possible re-insertion
    ...
    def decode(self, vector):
        ...
        m = self.m
        # we need a fixed random state for reproducibility; use vector itself to seed
        rng = np.random.RandomState(42)  # fixed seed for determinism across calls? Actually we want deterministic given vector only.
        for idx, (i, j) in enumerate(self.gap_positions):
            ...
            if v >= 0.8:   # deletion: replace gap with original character (or 'A' if not available)
                orig_char = self.orig_chars[i][j]
                aln[i][j] = orig_char if orig_char != '-' else 'A'
                continue
            if v <= -0.8:  # insertion: insert a new gap at a non-gap column chosen deterministically by vector
                # choose a column not already a gap; use the next vector value to decide which one
                non_gap_cols = [k for k in range(m) if aln[i][k] != '-']
                if non_gap_cols:
                    # use the fractional part of v to pick index
...
class EnhancedEO:
    ...
    def levy(self, size):
        v=np.random.randn(size); return 1/(np.abs(v)**(1/1.5)+1e-10)
    def eq_pool(self):
...
# ========================= Plotting functions =========================
def plot_convergence(hist_sac, hist_fix, initial_sp, filename='convergence.png'):
...
def plot_parameter_evolution(hist_sac, hist_fix, filename='parameter_evolution.png'):
    import matplotlib.pyplot as plt
...
def plot_correlation_heatmap(hist_sac, filename='heatmap.png'):
    ...
    # Drop constant columns (e.g., Gap Ratio)
