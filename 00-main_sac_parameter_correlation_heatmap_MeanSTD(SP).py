"""
MOFSACEO‑MSA with Gap‑Shift Representation (biologically valid)
Identical random initial populations for SAC and Fixed.
Repeats 5 runs, outputs mean±std table in Excel.
"""
...
# ========================= Gap‑Shift Representation =========================
class GapShiftRepresentation:
    def __init__(self, seed_alignment, max_shift=10):
...
# ========================= Enhanced EO =========================
class EnhancedEO:
    def __init__(self, dim, pop_size, bounds=(-1,1)):
        ...
        self.fit = np.zeros(pop_size); self.best = None; self.best_fit = -np.inf; self.stag = 0
    def levy(self, size):
        v = np.random.randn(size)
        return 1 / (np.abs(v) ** (1/1.5) + 1e-10)
    def eq_pool(self):
        idx = np.argsort(self.fit)[::-1]; top = self.pop[idx[:4]]
        return np.vstack([top, np.mean(top, axis=0)])
    def evolve(self, fitness_func, t, Tmax):
        T = t / Tmax
        for i in range(self.pop_size): self.fit[i] = fitness_func(self.pop[i])
        bi = np.argmax(self.fit)
        if self.fit[bi] > self.best_fit:
            self.best_fit = self.fit[bi]; self.best = self.pop[bi].copy(); self.stag = 0
        else: self.stag += 1
        eq = self.eq_pool()
        new = np.zeros_like(self.pop)
        for i in range(self.pop_size):
            xeq = eq[np.random.randint(len(eq))]
            lam = np.random.rand(self.dim); F = np.exp(-lam * T)
            r = np.random.rand(self.dim); G0 = (2*np.random.rand()-1)*(1-2*r)
...
class SAC:
    ...
    def state(self, eo_stats, sp, cs, gr, it, maxit):
        s = np.array([
            eo_stats['diversity'], eo_stats['mean_fitness']/1000,
            eo_stats['best_fitness']/1000, eo_stats['stagnation']/10,
            sp/100, cs, gr/100, it/maxit
        ], dtype=np.float32)
...
# ========================= MOFSACEO‑MSA =========================
class MOFSACEO_MSA:
    def __init__(self, seqs, seed_alignment, pop_size=200, max_iter=3000, verbose=False):
        self.verbose = verbose
        self.rep = GapShiftRepresentation(seed_alignment, max_shift=10)
        self.dim = self.rep.get_dimension()
        if verbose: print(f"Representation dimension: {self.dim}")
        # random initial population (same seed outside ensures identical)
        self.eo = EnhancedEO(self.dim, pop_size, bounds=(-1,1))
        self.sac = SAC(8,4)
    ...
    def run_sac(self):
        prev_sp = -np.inf
        hist = {'sp':[],'diversity':[],'levy':[],'pmut':[],'gap_ratio':[],'cs':[]}
        for t in range(1, self.max_iter+1):
            ...
            hist['sp'].append(sp); hist['diversity'].append(st['diversity'])
            hist['levy'].append(self.eo.alpha_levy); hist['pmut'].append(self.eo.p_mut)
            hist['gap_ratio'].append(gr); hist['cs'].append(cs)
            if self.verbose and (t % 500 == 0 or t == 1):
                print(f"Iter {t:5d}  SP={sp:6.1f}  diversity={st['diversity']:.3f}  α_levy={self.eo.alpha_levy:.4f}  p_mut={self.eo.p_mut:.3f}")
        return hist['sp'][-1], hist
    def run_fixed(self):
        self.eo = EnhancedEO(self.dim, len(self.eo.pop), bounds=(-1,1))
        hist = {'sp':[],'diversity':[],'levy':[],'pmut':[],'gap_ratio':[],'cs':[]}
        for t in range(1, self.max_iter+1):
            st = self.eo.evolve(self.fitness, t, self.max_iter)
            best_aln = self.decode(self.eo.best)
            ...
            if self.verbose and t % 500 == 0:
                print(f"Iter {t:5d}  SP={sp:6.1f}  diversity={st['diversity']:.3f}")
        return hist['sp'][-1], hist

# ========================= Helpers =========================
def convergence_iter(sp_history, threshold=0.95):
    final_max = max(sp_history)
    target = threshold * final_max
    for i, sp in enumerate(sp_history):
        if sp >= target:
            return i + 1
    return len(sp_history)

# ========================= Main Experiment =========================
def run_multiple():
    fasta_file = "input.fasta"
    if not os.path.exists(fasta_file):
        print("input.fasta missing"); return
    seqs = read_fasta(fasta_file)
    mafft_seed = run_mafft_seed(seqs)
    if not mafft_seed:
        mafft_seed = [s.ljust(max(len(s) for s in seqs), '-') for s in seqs]
    target_sp = sp_score(mafft_seed)
    print(f"Loaded {len(seqs)} sequences, MAFFT SP={target_sp}")
    ...
    for run_idx, seed in enumerate(seeds):
        ...
        # Set seed for identical initial population
        np.random.seed(seed); torch.manual_seed(seed); random.seed(seed)
        opt_sac = MOFSACEO_MSA(seqs, mafft_seed, pop_size=200, max_iter=3000, verbose=True)
        print("--- SAC ---")
        sp_sac, hist_sac = opt_sac.run_sac()
        sac_sps.append(sp_sac)
        sac_convs.append(convergence_iter(hist_sac['sp']))

        # Reset seed for fixed run (identical initial population)
        np.random.seed(seed); torch.manual_seed(seed); random.seed(seed)
        opt_fix = MOFSACEO_MSA(seqs, mafft_seed, pop_size=200, max_iter=3000, verbose=True)
        print("--- Fixed ---")
        sp_fix, hist_fix = opt_fix.run_fixed()
        fix_sps.append(sp_fix)
        fix_convs.append(convergence_iter(hist_fix['sp']))

    def mean_std(lst): return np.mean(lst), np.std(lst, ddof=1)
    sac_mean, sac_std = mean_std(sac_sps)
    fix_mean, fix_std = mean_std(fix_sps)
    sac_cm, sac_cs = mean_std(sac_convs)
    fix_cm, fix_cs = mean_std(fix_convs)

    results = [
        {'Method': 'MOFSACEO-MSA (SAC)',
         'SP (mean ± std)': f"{sac_mean:.1f} ± {sac_std:.1f}",
         'Convergence Iter (95%)': f"{sac_cm:.1f} ± {sac_cs:.1f}"},
        {'Method': 'EO (Fixed parameters)',
         'SP (mean ± std)': f"{fix_mean:.1f} ± {fix_std:.1f}",
         'Convergence Iter (95%)': f"{fix_cm:.1f} ± {fix_cs:.1f}"}
    ]
    ...
    print("Table saved to comparison_results.xlsx")
